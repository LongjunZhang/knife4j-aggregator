...
import DiffPage from '@/views/docAggregator/Diff.vue'
...
onMounted(() => {
  ...
  // 注册清空 Tab 的回调函数（供版本切换时调用）
  globalsStore.registerClearTabsCallback(clearAllTabs)
})
...
// 监听 currentMenuData 变化（版本切换时会触发）
watch(() => currentMenuData.value, (newMenuData) => {
  state.localMenuData = newMenuData;
}, { deep: true })
...
/**
 * 清空所有 Tab 页面（保留主页）
 * 版本切换时调用，防止用户访问旧版本的接口
 */
function clearAllTabs() {
  // 只保留主页 Tab
  state.panels = state.panels.filter(pane => pane.key === 'kmain');
  state.linkList = state.linkList.filter(key => key === 'kmain');
  // 切换到主页
  state.activeKey = 'kmain';
  // 更新主页的实例引用
  updateMainTabInstance();
