...
function SwaggerBootstrapUi(options) {
  ...
  if (this.springdoc) {
    ...
    // FIX: 确保 basePath 至少是 '/'，避免生成没有前导 / 的 URL
    let basePath = path.length == index + 1 ? path : path.substring(0, index);
...
SwaggerBootstrapUi.prototype.analysisSpringDocOpenApiGroupSuccess = function (data) {
  ...
  if (KUtils.arrNotEmpty(groupUrls)) {
    groupUrls.forEach(gu => {
      var newGroup = {
        ...
        // 支持网关聚合场景下的 servicePath/contextPath
  ...
  newGroupData.forEach(function (group) {
    ...
    g.url = group.url;
    // 设置 servicePath 支持网关聚合场景下的调试功能
    g.servicePath = group.servicePath || group.contextPath;
...
SwaggerBootstrapUi.prototype.createApiInfoInstance = function (path, mtype, apiInfo, appendBaePathFlag) {
  ...
  if (KUtils.strNotBlank(that.currentInstance.servicePath)) {
    // 兼容性处理：
    // - 对于部分 /v2/api-docs（Swagger2 -> OpenAPI3 转换）场景，paths 的 key 已经包含了 servicePath 前缀（如 /messageService/message）
    // - 此时再拼接一次 servicePath 会导致 UI 显示为 /messageService/messageService/message
    // 规则：如果 newfullPath 已经以 servicePath 开头，则不重复拼接
    var servicePath = that.currentInstance.servicePath;
    // 规范化
    var sp = servicePath.endsWith('/') ? servicePath.substring(0, servicePath.length - 1) : servicePath;
    var nf = newfullPath.startsWith('/') ? newfullPath : ('/' + newfullPath);

    if (nf === sp || nf.startsWith(sp + '/')) {
      newurl = nf;
    } else {
      var tempUrl = sp;
      tempUrl = tempUrl + nf;
