    ...
    <a-spin tip="Loading..." :spinning="debugLoading">
      <div class="spin-content">
        ...
        <a-row class="knife4j-debug-tabs">
          <a-tabs v-model:activeKey="debugActiveKey" @change="handleDebugTabChange">
            <a-tab-pane key="1">
              <template #tab>
                <span>
            ...
            <a-tab-pane :tab="$t('debug.params')" key="2" forceRender>
              <div class="knife4j-debug-request-type">
                <div class="knife4j-debug-request-content-type-float">
                  ...
                  <a-dropdown v-if="rawTypeFlag" class="knife4j-debug-raw-dropdown">
              ...
              <a-row v-if="rawFlag">
                ...
                <editor-debug-show ref="rawEditor" style="margin-top:5px;" v-model:value="rawText" :mode="rawMode">
                </editor-debug-show>
              </a-row>
            </a-tab-pane>
            ...
            <!-- AI 生成参数按钮 (作为 Tab 展示) -->
            <a-tab-pane key="ai-generate">
              <template #tab>
                <a-button type="primary" size="small" shape="round" :loading="aiGenerating" style="background: linear-gradient(to right, #1890ff, #722ed1); border: none; font-size: 12px;">
...
import { UnlockOutlined, DownOutlined, ThunderboltOutlined } from '@ant-design/icons-vue'
import { generateMockData } from '@/services/schemaFakerService.js'
...
export default {
  ...
  setup() {
    ...
    // 当前 API 文档版本（用于调试请求时携带版本信息）
    const currentDocVersion = computed(() => {
      return globalsStore.currentVersion;
    })
    // 当前服务名
    const currentServiceName = computed(() => {
      return globalsStore.currentServiceName;
    })
  ...
  data() {
    return {
      ...
      oAuthApi: false,
      debugSend: false,
      // 响应头中的错误 ID（由 error-collector 返回）
  ...
  methods: {
    ...
    handleDebugTabChange(activeKey) {
      if (activeKey === 'ai-generate') {
        // 点击 AI 生成参数按钮时，强制切换到请求参数 Tab
        this.$nextTick(() => {
          this.debugActiveKey = '2';
        });
        // 执行生成逻辑
        this.generateParams();
        return;
     ...
     * 快速生成测试参数
     * 
     * 使用 json-schema-faker 在前端本地生成模拟数据，
     * 无需调用后端接口，毫秒级响应。
     */
    generateParams() {
      ...
      try {
        // 调用前端 Schema Faker 服务生成模拟数据
        const mockData = generateMockData(this.api, this.swaggerInstance);
        
        let hasData = false;
        
        // 1. 处理 form-data 类型
        if (this.requestContentType === 'form-data') {
          let targetData = mockData.body;
          // 如果 body 为空，尝试使用 queryParams (兼容 GET 等无 body 情况)
          if (!targetData || (typeof targetData === 'object' && Object.keys(targetData).length === 0)) {
            targetData = mockData.queryParams;
          }
          
          if (targetData && typeof targetData === 'object' && Object.keys(targetData).length > 0) {
            this.formData = this.fillFormParams(this.formData, targetData);
            // 重置选中状态并重新计算必填项 (解决必选框丢失问题)
            this.rowFormSelection.selectedRowKeys = [];
            this.initFormSelections();
            hasData = true;
          }
        }
        // 2. 处理 x-www-form-urlencoded 类型
        else if (this.requestContentType === 'x-www-form-urlencoded') {
          let targetData = mockData.body;
          if (!targetData || (typeof targetData === 'object' && Object.keys(targetData).length === 0)) {
            targetData = mockData.queryParams;
          }
          ...
          if (targetData && typeof targetData === 'object' && Object.keys(targetData).length > 0) {
            this.urlFormData = this.fillFormParams(this.urlFormData, targetData);
            // 重置选中状态并重新计算必填项 (解决必选框丢失问题)
            this.rowUrlFormSelection.selectedRowKeys = [];
            this.initUrlFormSelections();
            hasData = true;
          }
        }
        // 3. 处理 Raw 类型 (或其他默认情况)
          ...
          // 回填请求体到 rawText
          if (mockData.body !== null && mockData.body !== undefined) {
            const newRawText = typeof mockData.body === 'object' 
              ? JSON.stringify(mockData.body, null, 2)
              : String(mockData.body);
            ...
            // 直接调用编辑器组件的方法强制刷新
            this.$nextTick(() => {
              if (this.$refs.rawEditor && this.$refs.rawEditor.editor) {
                this.$refs.rawEditor.editor.setValue(newRawText, -1);
                this.$refs.rawEditor.resetEditorHeight();
              }
            });
            
            // 切换到 raw 模式（仅当有 body 数据时）
            if (this.rawFlag || Object.keys(mockData.body).length > 0) {
              this.showTabRaw();
            }
            hasData = true;
          }
          
          // 回填 query 参数到 rawFormData (Raw 模式下的参数表)
          if (mockData.queryParams && Object.keys(mockData.queryParams).length > 0) {
            this.rawFormData = this.fillFormParams(this.rawFormData, mockData.queryParams);
            // 刷新选中状态
            this.rowRawFormSelection.selectedRowKeys = [];
            this.initRawFormSelections();
            hasData = true;
          }
        }
        ...
        // 回填 path 参数到 URL (通用)
        if (mockData.pathParams && Object.keys(mockData.pathParams).length > 0) {
          this.fillPathParams(mockData.pathParams);
          hasData = true;
        }
        
        // 回填 headers (通用)
        if (mockData.headers && Object.keys(mockData.headers).length > 0) {
          this.headerData = this.fillFormParams(this.headerData, mockData.headers);
          hasData = true;
        }
        ...
        if (hasData) {
          ...
          message.info('未找到可生成的参数定义');
        }
      } catch (error) {
        console.error('参数生成错误:', error);
        message.error('参数生成失败: ' + (error.message || '未知错误'));
      } finally {
    ...
    /**
     * 填充表单参数
     * 返回新数组以触发 Vue 响应式更新，强制重新渲染表格
     * 
     * @param {Array} formData - 表单数据数组
     * @param {Object} params - 参数键值对
     * @returns {Array} 更新后的表单数据数组
     */
    fillFormParams(formData, params) {
      if (!formData || !params) return formData || [];
      
      // 创建新数组，同时生成新的 id 以强制 Vue 重新渲染组件
      // 因为 a-input 使用 :defaultValue 而不是 v-model
      return formData.map(item => {
        if (params.hasOwnProperty(item.name)) {
          return {
            ...item,
            // 生成新的 id 强制重新渲染输入框
            id: KUtils.md5Id({ name: item.name, timestamp: Date.now(), random: Math.random() }),
            content: String(params[item.name])
          };
        }
        return { ...item };
      });
    ...
    /**
     * 填充路径参数到 URL
     * @param {Object} pathParams - 路径参数键值对
     */
    fillPathParams(pathParams) {
      if (!pathParams) return;
      
      let url = this.debugUrl;
    ...
    debugHeaders() {
      ...
      // 追加 API 文档版本号（用于后端校验接口是否存在于当前版本）
      if (this.currentDocVersion) {
        headers["X-Doc-Version"] = String(this.currentDocVersion);
      }
      if (this.currentServiceName) {
        headers["X-Doc-Service"] = String(this.currentServiceName);
      }
    ...
    debugSendUrlFormRequest() {
      ...
      if (validateForm.validate) {
        ...
        // 获取调试请求的 baseURL
        let baseUrl = this.getDebugBaseUrl();
        // console.log(headers)
    ...
    handleDebugSuccess(startTime, endTime, res) {
      ...
      this.storeApiParams();
      // 强制切换到响应内容 Tab
      this.$refs.childDebugResponse?.resetToResponseTab?.();
    },
    ...
    setResponseHeaders(respHeaders) {
      ...
      if (KUtils.checkUndefined(respHeaders)) {
        for (var k in respHeaders) {
          ...
          // 提取 X-Error-Id 响应头
          if (k.toLowerCase() === 'x-error-id') {
            this.responseErrorId = respHeaders[k];
          }
        }
      }
